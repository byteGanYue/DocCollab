...
import { Select, Button, Card, Space, Typography, Spin, message } from 'antd';
...
const VersionCompare = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            try {
                const historyRes = await documentAPI.getDocumentHistory(documentId, 1, 100);
    ...
    useEffect(() => {
        async function fetchContent(versionId, setter) {
            ...
            const res = await documentAPI.getDocumentVersion(documentId, versionId, userId);
        ...
        <div style={{ padding: 32, background: '#f7f8fa', minHeight: '100vh' }}>
            <Space align="center" style={{ marginBottom: 24 }}>
                ...
                <Title level={3} style={{ margin: 0 }}>
                    <DiffOutlined style={{ color: '#1890ff', marginRight: 8 }} />版本结构化对比
                </Title>
            </Space>
            ...
            <Spin spinning={loading} tip="加载中...">
                <div style={{ display: 'flex', gap: 24 }}>
                    <Card title={<span>左侧版本内容</span>} style={{ flex: 1, minHeight: 600 }}>
                        {leftContent !== null ? (
                            <VersionDiff
                                oldContent={typeof leftContent === 'string' && leftContent.trim() ? leftContent : '[]'}
                                newContent={typeof rightContent === 'string' && rightContent?.trim() ? rightContent : '[]'}
                                mode="left"
                    ...
                    <Card title={<span>右侧版本内容</span>} style={{ flex: 1, minHeight: 600 }}>
                        {rightContent !== null ? (
                            <VersionDiff
                                oldContent={typeof leftContent === 'string' && leftContent?.trim() ? leftContent : '[]'}
                                newContent={typeof rightContent === 'string' && rightContent.trim() ? rightContent : '[]'}
